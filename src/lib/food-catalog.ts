 ...
 *
 * Per-100g values are standard **USDA FoodData Central** reference figures for
 * generic foods; a few Colombian/prepared items (arepa, queso campesino, whey)
 * carry an explicit "estimate" `source` because they vary by recipe/brand. Every
 * food records its `source`. Values are educational — users can edit before
 * logging.
 */
...
export type CatalogFood = {
  ...
  /** Canonical nutrition per 100 g (fiber/sugar/sat-fat grams; sodium mg). */
  per100g: {
    calories: number;
    protein: number;
    carbs: number;
  ...
  /** Provenance of the values (USDA FoodData Central, or an explicit estimate). */
  source: string;
...
export const FOOD_CATALOG: CatalogFood[] = [
  ...
  },
  {
    ...
    source: "Estimate (varies by brand)",
  ...
  },
  {
    ...
    source: "Estimate (varies by recipe)",
  ...
  {
    slug: "greek-yogurt",
    name: "Greek yogurt, nonfat (Yogur griego)",
    category: "dairy",
    per100g: {
      calories: 59,
      protein: 10,
  ...
  },
  {
    ...
    source: "Estimate (queso fresco, varies)",
